"use client";
import classes from "./Washers.module.scss";
...
// JSON DATA
import WashersJson from "../../../../../../../Json_Data/Electronics/Washers.json";
...
export default function Washers({
	...
	const [listingDetails, setListingDetails] = useState({
		[String(WashersJson[0].labelEn).toLocaleLowerCase()]: {
			labelAr: WashersJson[0].labelAr,
			labelEn: WashersJson[0].labelEn,
			value: "",
		},
		[String(WashersJson[1].labelEn).toLocaleLowerCase()]: {
			labelAr: WashersJson[1].labelAr,
			labelEn: WashersJson[1].labelEn,
			value: "",
		},
		[String(WashersJson[2].labelEn).toLocaleLowerCase()]: {
			labelAr: WashersJson[2].labelAr,
			labelEn: WashersJson[2].labelEn,
			value: "",
		},
		[String(WashersJson[3].labelEn).toLocaleLowerCase()]: {
			labelAr: WashersJson[3].labelAr,
			labelEn: WashersJson[3].labelEn,
			value: "",
		},
	});
	...
	const modelsOptions = WashersJson[0].Values.map((value) => {
		return {
	...
	const modelsOptions2 = WashersJson[1].Values.map((value) => {
		return {
			label: locale === "en" ? value.labelEn : value.labelAr,
			value: value.labelEn,
		};
		...
		<div className={`${classes.SpareParts} rounded p-4`}>
			...
			>
				...
				<div className={`${classes.Type} rounded bg-white mt-4`}>
					<h3>
						{locale === "en"
							? WashersJson[0].labelEn
							: WashersJson[0].labelAr}
					</h3>
				...
				<div className={`${classes.Type} rounded bg-white mt-4`}>
					<h3>
						{locale === "en"
							? WashersJson[1].labelEn
							: WashersJson[1].labelAr}
					</h3>
					<div className={"flex justify-start gap-2 flex-wrap"}>
						<DropDown
							value={
				...
				<div className={`${classes.Type} rounded bg-white mt-4`}>
					<h3>
						{locale === "en"
							? WashersJson[2].labelEn
							: WashersJson[2].labelAr}
					</h3>
					<div className={"flex justify-start gap-2 flex-wrap"}>
						{WashersJson[2].Values.map((value, index) => {
							return (
								<RadioComponent
									key={index}
									locale={locale}
									...
									name={String(WashersJson[2].labelEn)}
									onChange={(event) => {
										setListingDetails({
											...listingDetails,
											[String(
				...
				<div className={`${classes.Type} rounded bg-white mt-4`}>
					<h3>
						{locale === "en"
							? WashersJson[3].labelEn
							: WashersJson[3].labelAr}
					</h3>
					<div className={"flex justify-start gap-2 flex-wrap"}>
						{WashersJson[3].Values.map((value, index) => {
							return (
								<RadioComponent
									key={index}
									locale={locale}
									...
									textAr={value.labelAr}
									textEn={value.labelEn}
									name={String(WashersJson[3].labelEn)}
									onChange={(event) => {
										setListingDetails({
											...listingDetails,
											[String(
