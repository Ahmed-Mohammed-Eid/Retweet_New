"use client";
import classes from "./DiningRoomAndFurniture.module.scss";
...
export default function DiningRoomAndFurniture({
	locale,
	categoryName,
	subCategoryName,
	submit = () => {},
	...
	const [listingDetails, setListingDetails] = useState({
		[String(DiningRoomAndFurnitureJson[0].labelEn).toLocaleLowerCase()]: {
			labelAr: DiningRoomAndFurnitureJson[0].labelAr,
			labelEn: DiningRoomAndFurnitureJson[0].labelEn,
			value: "",
		},
	});
	// LOCATION PART
			...
			>
				...
				<div className={`${classes.Delivery} rounded bg-white`}>
					<h3 className={"uppercase"}>
						{locale === "en"
							? DiningRoomAndFurnitureJson[0].labelEn
							: DiningRoomAndFurnitureJson[0].labelAr}
					</h3>

					<div className={"flex justify-start gap-2 flex-wrap"}>
						{DiningRoomAndFurnitureJson[0].Values.map(
							(value, index) => (
								<RadioComponent
									key={index}
									locale={locale}
