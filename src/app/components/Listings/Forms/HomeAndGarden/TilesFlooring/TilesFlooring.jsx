"use client";
import classes from "./TilesFlooring.module.scss";
...
// JSON DATA
import TilesFlooringJson from "../../../../../../../Json_Data/HouseAndGarden/TilesFlooring.json";
...
export default function TilesFlooring({
	...
	const [listingDetails, setListingDetails] = useState({
		[String(TilesFlooringJson[0].labelEn).toLocaleLowerCase()]: {
			labelAr: TilesFlooringJson[0].labelAr,
			labelEn: TilesFlooringJson[0].labelEn,
			value: "",
		},
		[String(TilesFlooringJson[1].labelEn).toLocaleLowerCase()]: {
			labelAr: TilesFlooringJson[1].labelAr,
			labelEn: TilesFlooringJson[1].labelEn,
			value: "",
		},
	});
	// LOCATION PART
		...
		<div className={`${classes.SpareParts} rounded p-4`}>
			...
			>
				...
				<div className={`${classes.Type} rounded bg-white mt-4`}>
					<h3>
						{locale === "en"
							? TilesFlooringJson[0].labelEn
							: TilesFlooringJson[0].labelAr}
					</h3>
					<div className={"flex justify-start gap-2 flex-wrap"}>
						{TilesFlooringJson[0].Values.map((value, index) => (
							<RadioComponent
								key={index}
								locale={locale}
								value={value.labelEn}
								...
								name={String(TilesFlooringJson[0].labelEn)}
								onChange={(event) => {
									setListingDetails({
										...listingDetails,
										[String(
				...
				<div className={`${classes.Delivery} rounded bg-white`}>
					<h3 className={"uppercase"}>
						{locale === "en"
							? TilesFlooringJson[1].labelEn
							: TilesFlooringJson[1].labelAr}
					</h3>

					<div className={"flex justify-start gap-2 flex-wrap"}>
						{TilesFlooringJson[1].Values.map((value, index) => (
							<RadioComponent
								key={index}
								locale={locale}
								value={value.labelEn}
								textAr={value.labelAr}
								textEn={value.labelEn}
								name={String(TilesFlooringJson[1].labelEn)}
								onChange={(event) => {
									setListingDetails({
										...listingDetails,
										[String(
