"use client";
import classes from "./HouseKitchenTools.module.scss";
...
// JSON DATA
import HousekitchenTools from "../../../../../../../Json_Data/HouseAndGarden/HousekitchenTools.json";

export default function HouseKitchenTools({
	locale,
	categoryName,
	subCategoryName,
	submit = () => {},
	...
	const [listingDetails, setListingDetails] = useState({
		[String(HousekitchenTools[0].labelEn).toLocaleLowerCase()]: {
			labelAr: HousekitchenTools[0].labelAr,
			labelEn: HousekitchenTools[0].labelEn,
			value: "",
		},
		[String(HousekitchenTools[1].labelEn).toLocaleLowerCase()]: {
			labelAr: HousekitchenTools[1].labelAr,
			labelEn: HousekitchenTools[1].labelEn,
			value: "",
		},
		[String(HousekitchenTools[2].labelEn).toLocaleLowerCase()]: {
			labelAr: HousekitchenTools[2].labelAr,
			labelEn: HousekitchenTools[2].labelEn,
			value: "",
		},
	});
	// LOCATION PART
		...
		<div className={`${classes.SpareParts} rounded p-4`}>
			...
			>
				...
				<div className={`${classes.Type} rounded bg-white mt-4`}>
					<h3>
						{locale === "en"
							? HousekitchenTools[0].labelEn
							: HousekitchenTools[0].labelAr}
					</h3>
					<div className={"flex justify-start gap-2 flex-wrap"}>
						{HousekitchenTools[0].Values.map((value, index) => (
							<RadioComponent
								key={index}
								locale={locale}
								value={value.labelEn}
								...
								name={String(HousekitchenTools[0].labelEn)}
								onChange={(event) => {
									setListingDetails({
										...listingDetails,
										[String(
				...
				<div className={`${classes.Delivery} rounded bg-white`}>
					<h3 className={"uppercase"}>
						{locale === "en"
							? HousekitchenTools[1].labelEn
							: HousekitchenTools[1].labelAr}
					</h3>

					<div className={"flex justify-start gap-2 flex-wrap"}>
						{HousekitchenTools[1].Values.map((value, index) => (
							<RadioComponent
								key={index}
								locale={locale}
								value={value.labelEn}
								textAr={value.labelAr}
								textEn={value.labelEn}
								name={String(HousekitchenTools[1].labelEn)}
								onChange={(event) => {
									setListingDetails({
										...listingDetails,
										[String(
				...
				<div className={`${classes.Condition} rounded bg-white`}>
					<h3 className={"uppercase"}>
						{locale === "en"
							? HousekitchenTools[2].labelEn
							: HousekitchenTools[2].labelAr}
					</h3>

					<div className={"flex justify-start gap-2 flex-wrap"}>
						{HousekitchenTools[2].Values.map((value, index) => (
							<RadioComponent
								key={index}
								locale={locale}
								value={value.labelEn}
								textAr={value.labelAr}
								textEn={value.labelEn}
								name={String(HousekitchenTools[2].labelEn)}
								onChange={(event) => {
									setListingDetails({
										...listingDetails,
										[String(
