"use client";
import classes from "./Bathrooms.module.scss";
...
// JSON DATA
import BathroomsJson from "../../../../../../../Json_Data/HouseAndGarden/Bathrooms.json";
...
export default function Bathrooms({
	...
	const [listingDetails, setListingDetails] = useState({
		[String(BathroomsJson[0].labelEn).toLocaleLowerCase()]: {
			labelAr: BathroomsJson[0].labelAr,
			labelEn: BathroomsJson[0].labelEn,
			value: "",
		},
		[String(BathroomsJson[1].labelEn).toLocaleLowerCase()]: {
			labelAr: BathroomsJson[1].labelAr,
			labelEn: BathroomsJson[1].labelEn,
			value: "",
		},
	});
	// LOCATION PART
		...
		<div className={`${classes.SpareParts} rounded p-4`}>
			...
			>
				...
				<div className={`${classes.Type} rounded bg-white mt-4`}>
					<h3>
						{locale === "en"
							? BathroomsJson[0].labelEn
							: BathroomsJson[0].labelAr}
					</h3>
					<div className={"flex justify-start gap-2 flex-wrap"}>
						{BathroomsJson[0].Values.map((value, index) => (
							<RadioComponent
								key={index}
								locale={locale}
								value={value.labelEn}
								...
								name={String(BathroomsJson[0].labelEn)}
								onChange={(event) => {
									setListingDetails({
										...listingDetails,
										[String(
				...
				<div className={`${classes.Delivery} rounded bg-white`}>
					<h3 className={"uppercase"}>
						{locale === "en"
							? BathroomsJson[1].labelEn
							: BathroomsJson[1].labelAr}
					</h3>

					<div className={"flex justify-start gap-2 flex-wrap"}>
						{BathroomsJson[1].Values.map((value, index) => (
							<RadioComponent
								key={index}
								locale={locale}
								value={value.labelEn}
								textAr={value.labelAr}
								textEn={value.labelEn}
								name={String(BathroomsJson[1].labelEn)}
								onChange={(event) => {
									setListingDetails({
										...listingDetails,
										[String(
