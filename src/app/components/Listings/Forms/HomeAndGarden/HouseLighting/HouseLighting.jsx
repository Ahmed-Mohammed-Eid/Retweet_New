"use client";
import classes from "./HouseLighting.module.scss";
...
// JSON DATA
import HouseLightingJson from "../../../../../../../Json_Data/HouseAndGarden/HouseLighting.json";

export default function HouseLighting({
	locale,
	categoryName,
	subCategoryName,
	submit = () => {},
	...
	const [listingDetails, setListingDetails] = useState({
		[String(HouseLightingJson[0].labelEn).toLocaleLowerCase()]: {
			labelAr: HouseLightingJson[0].labelAr,
			labelEn: HouseLightingJson[0].labelEn,
			value: "",
		},
		[String(HouseLightingJson[1].labelEn).toLocaleLowerCase()]: {
			labelAr: HouseLightingJson[1].labelAr,
			labelEn: HouseLightingJson[1].labelEn,
			value: "",
		},
		[String(HouseLightingJson[2].labelEn).toLocaleLowerCase()]: {
			labelAr: HouseLightingJson[2].labelAr,
			labelEn: HouseLightingJson[2].labelEn,
			value: "",
		},
	});
	// LOCATION PART
		...
		<div className={`${classes.SpareParts} rounded p-4`}>
			...
			>
				...
				<div className={`${classes.Type} rounded bg-white mt-4`}>
					<h3>
						{locale === "en"
							? HouseLightingJson[0].labelEn
							: HouseLightingJson[0].labelAr}
					</h3>
					<div className={"flex justify-start gap-2 flex-wrap"}>
						{HouseLightingJson[0].Values.map((value, index) => (
							<RadioComponent
								key={index}
								locale={locale}
								value={value.labelEn}
								...
								name={String(HouseLightingJson[0].labelEn)}
								onChange={(event) => {
									setListingDetails({
										...listingDetails,
										[String(
				...
				<div className={`${classes.Delivery} rounded bg-white`}>
					<h3 className={"uppercase"}>
						{locale === "en"
							? HouseLightingJson[1].labelEn
							: HouseLightingJson[1].labelAr}
					</h3>

					<div className={"flex justify-start gap-2 flex-wrap"}>
						{HouseLightingJson[1].Values.map((value, index) => (
							<RadioComponent
								key={index}
								locale={locale}
								value={value.labelEn}
								textAr={value.labelAr}
								textEn={value.labelEn}
								name={String(HouseLightingJson[1].labelEn)}
								onChange={(event) => {
									setListingDetails({
										...listingDetails,
										[String(
				...
				<div className={`${classes.Condition} rounded bg-white`}>
					<h3>
						{locale === "en"
							? HouseLightingJson[2].labelEn
							: HouseLightingJson[2].labelAr}
					</h3>
					<div className={"flex justify-start gap-2 flex-wrap"}>
						{HouseLightingJson[2].Values.map((value, index) => (
							<RadioComponent
								key={index}
								locale={locale}
								value={value.labelEn}
								textAr={value.labelAr}
								textEn={value.labelEn}
								name={String(HouseLightingJson[2].labelEn)}
								onChange={(event) => {
									setListingDetails({
										...listingDetails,
										[String(
