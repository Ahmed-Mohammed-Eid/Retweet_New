"use client";
import classes from "./Printers.module.scss";
...
// JSON DATA
import PrintersJson from "../../../../../../../Json_Data/Laptops/Printers.json";
...
export default function Printers({
	...
	const [listingDetails, setListingDetails] = useState({
		[String(PrintersJson[0].labelEn).toLocaleLowerCase()]: {
			labelAr: PrintersJson[0].labelAr,
			labelEn: PrintersJson[0].labelEn,
			value: "",
		},
		[String(PrintersJson[1].labelEn).toLocaleLowerCase()]: {
			labelAr: PrintersJson[1].labelAr,
			labelEn: PrintersJson[1].labelEn,
			value: "",
		},
		[String(PrintersJson[2].labelEn).toLocaleLowerCase()]: {
			labelAr: PrintersJson[2].labelAr,
			labelEn: PrintersJson[2].labelEn,
			value: "",
		},
		[String(PrintersJson[3].labelEn).toLocaleLowerCase()]: {
			labelAr: PrintersJson[3].labelAr,
			labelEn: PrintersJson[3].labelEn,
			value: "",
		},
	});
	...
	const modelsOptions = PrintersJson[0].Values.map((value) => {
		return {
	...
	const modelsOptions2 = PrintersJson[1].Values.map((value) => {
		return {
			label: locale === "en" ? value.labelEn : value.labelAr,
			value: value.labelEn,
		};
		...
		<div className={`${classes.SpareParts} rounded p-4`}>
			...
			>
				...
				<div className={`${classes.Type} rounded bg-white mt-4`}>
					<h3>
						{locale === "en"
							? PrintersJson[0].labelEn
							: PrintersJson[0].labelAr}
					</h3>
				...
				<div className={`${classes.Type} rounded bg-white mt-4`}>
					<h3>
						{locale === "en"
							? PrintersJson[1].labelEn
							: PrintersJson[1].labelAr}
					</h3>
					<div className={"flex justify-start gap-2 flex-wrap"}>
						<DropDown
							value={
				...
				<div className={`${classes.Type} rounded bg-white mt-4`}>
					<h3>
						{locale === "en"
							? PrintersJson[2].labelEn
							: PrintersJson[2].labelAr}
					</h3>
					<div className={"flex justify-start gap-2 flex-wrap"}>
						{PrintersJson[2].Values.map((value, index) => {
							return (
								<RadioComponent
									key={index}
									locale={locale}
									...
									name={String(PrintersJson[2].labelEn)}
									onChange={(event) => {
										setListingDetails({
											...listingDetails,
											[String(
				...
				<div className={`${classes.Type} rounded bg-white mt-4`}>
					<h3>
						{locale === "en"
							? PrintersJson[3].labelEn
							: PrintersJson[3].labelAr}
					</h3>
					<div className={"flex justify-start gap-2 flex-wrap"}>
						{PrintersJson[3].Values.map((value, index) => {
							return (
								<RadioComponent
									key={index}
									locale={locale}
									...
									textAr={value.labelAr}
									textEn={value.labelEn}
									name={String(PrintersJson[3].labelEn)}
									onChange={(event) => {
										setListingDetails({
											...listingDetails,
											[String(
