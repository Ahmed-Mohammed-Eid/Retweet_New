...
// JSON DATA
import PhoneNumbersJson from "../../../../../../../Json_Data/VideoGames/accessories.json";
...
export default function Accessories({
	...
	const [listingDetails, setListingDetails] = useState({
		...
		[String(PhoneNumbersJson[3].labelEn).toLocaleLowerCase()]: {
			labelAr: PhoneNumbersJson[3].labelAr,
			labelEn: PhoneNumbersJson[3].labelEn,
			value: "",
		},
	});
	// LOCATION PART
		...
		<div className={`${classes.SpareParts} rounded p-4`}>
			...
			>
				...
				<div className={`${classes.Brand} rounded bg-white`}>
					<h3 className={"uppercase"}>
						{locale === "en"
							? PhoneNumbersJson[3].labelEn
							: PhoneNumbersJson[3].labelAr}
					</h3>

					<div className={"flex justify-start gap-2 flex-wrap"}>
						{PhoneNumbersJson[3].Values.map((value, index) => (
							<RadioComponent
								key={index}
								locale={locale}
								value={value.labelEn}
								textAr={value.labelAr}
								textEn={value.labelEn}
								name={String(PhoneNumbersJson[3].labelEn)}
								onChange={(event) => {
									setListingDetails({
										...listingDetails,
										[String(
