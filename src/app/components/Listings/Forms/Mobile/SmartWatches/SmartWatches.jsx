"use client";
import classes from "./SmartWatches.module.scss";
...
// JSON DATA
import SmartWatchesJson from "../../../../../../../Json_Data/mobilesAndTablets/smartWatches.json";
...
export default function SmartWatches({
	...
	const [listingDetails, setListingDetails] = useState({
		[String(SmartWatchesJson[0].labelEn).toLocaleLowerCase()]: {
			labelAr: SmartWatchesJson[0].labelAr,
			labelEn: SmartWatchesJson[0].labelEn,
			value: "",
		},
		[String(SmartWatchesJson[1].labelEn).toLocaleLowerCase()]: {
			labelAr: SmartWatchesJson[1].labelAr,
			labelEn: SmartWatchesJson[1].labelEn,
			value: "",
		},
		[String(SmartWatchesJson[2].labelEn).toLocaleLowerCase()]: {
			labelAr: SmartWatchesJson[2].labelAr,
			labelEn: SmartWatchesJson[2].labelEn,
			value: "",
		},
	});
	// LOCATION PART
		...
		<div className={`${classes.SpareParts} rounded p-4`}>
			...
			>
				...
				<div className={`${classes.Type} rounded bg-white mt-4`}>
					<h3>
						{locale === "en"
							? SmartWatchesJson[0].labelEn
							: SmartWatchesJson[0].labelAr}
					</h3>
					<div className={"flex justify-start gap-2 flex-wrap"}>
						{SmartWatchesJson[0].Values.map((value, index) => (
							<RadioComponent
								key={index}
								locale={locale}
								value={value.labelEn}
								...
								name={String(SmartWatchesJson[0].labelEn)}
								onChange={(event) => {
									setListingDetails({
										...listingDetails,
										[String(
				...
				<div className={`${classes.Delivery} rounded bg-white`}>
					<h3 className={"uppercase"}>
						{locale === "en"
							? SmartWatchesJson[1].labelEn
							: SmartWatchesJson[1].labelAr}
					</h3>

					<div className={"flex justify-start gap-2 flex-wrap"}>
						{SmartWatchesJson[1].Values.map((value, index) => (
							<RadioComponent
								key={index}
								locale={locale}
								value={value.labelEn}
								textAr={value.labelAr}
								textEn={value.labelEn}
								name={String(SmartWatchesJson[1].labelEn)}
								onChange={(event) => {
									setListingDetails({
										...listingDetails,
										[String(
				...
				<div className={`${classes.Condition} rounded bg-white`}>
					<h3 className={"uppercase"}>
						{locale === "en"
							? SmartWatchesJson[2].labelEn
							: SmartWatchesJson[2].labelAr}
					</h3>

					<div className={"flex justify-start gap-2 flex-wrap"}>
						{SmartWatchesJson[2].Values.map((value, index) => (
							<RadioComponent
								key={index}
								locale={locale}
								value={value.labelEn}
								textAr={value.labelAr}
								textEn={value.labelEn}
								name={String(SmartWatchesJson[2].labelEn)}
								onChange={(value) => {
									setListingDetails({
										...listingDetails,
										[String(
